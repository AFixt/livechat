...
describe('test environment — Web Storage (issue #153)', () => {
  ...
  it('exposes a usable window.sessionStorage, kept separate from localStorage', () => {
    ...
    // A single shared store would make the ADR-0013 "not mirrored into
    // sessionStorage" assertions in store/auth.test.ts vacuous.
    expect(window.localStorage.getItem('livechat.probe')).toBeNull();
  });

  it('reaches the same store through the bare `localStorage` binding', () => {
    // `services/alert-sound.ts` reads the unqualified global rather than
    // `window.localStorage`, so the two access paths must name one store —
    // written through one, read back through the other.
