...
export function buildVisitorRouter(deps: VisitorRouterDeps): Router {
  ...
  router.post(
    '/chats/:id/transcript',
    // Cookie-authenticated and state-changing (it sends mail), so it carries
    // the same CSRF guard as the other visitor writes (#77).
    csrfProtection({ COOKIE_SECRET: deps.env.COOKIE_SECRET }),
    ...
    asyncHandler(async (req, res) => {
      ...
      // visitor, so one visitor cannot email another's transcript. This route
      // originally checked ownership inline because #72 had not landed yet.
