...
user-agent, and — when the client passes an identity token — the token's subject
claim. It is **not** a user account; there is no name, email or password.
...
- **IP truncation** — the last octet of an IPv4 address (`203.0.113.42` →
  `203.0.113.0`) and the last 80 bits of an IPv6 address (`2001:db8:85a3:…` →
  `2001:db8:85a3::`, the `/48` prefix) are zeroed. The full address is never
...
`api/src/services/data-retention-service.ts` implements
`purgeExpiredVisitorData`; `api/src/scripts/purge-expired-visitor-data.ts` runs
it from env.
...
To erase a specific visitor ahead of the window, delete their `visitor_sessions`
row directly (cascades to their chats), or run the job with a short
`VISITOR_DATA_RETENTION_DAYS`. Because IPs are truncated and geo is
